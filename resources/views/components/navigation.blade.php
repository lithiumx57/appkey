<nav>
  <div class="header-icons mobile-show desktop-hidden">
    <span class="nav-toggle">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="24px" class="menu-icon">
      <path
  ...
  <div class="main-navigation">
    <div class="mobile-nav-header mobile-show desktop-hidden">
      <div id="mobile-menu-title">
        منوی اصلی
      </div>
      <div>
        <x-font-icons.times width="24" height="24" clazz="close-nav"/>
      </div>
    </div>

    <ul id="main-menu-container">
      @foreach(\App\Helpers\StaticData\Menus::getAll() as $key => $row)
        <li class="bold menu-1 main-categories {{$row["class"]}}" data-target="{{$key}}">
          <a href="{{$row["link"]}}" data-target="{{$key}}">
            <x-font-icons.dounble-angle-left width="12" height="12" clazz="desktop-hidden"/>
            <span data-target="{{$key}}">
             {{$row["title"]}}
            </span>
          </a>
          <ul class="submenu" id="submenu_{{$key}}">
            <ul>

              <li id="back-to-main-menu" class="desktop-hidden">
                <x-font-icons.dounble-angle-left width="16" height="16" class="back-icon"/>
                بازگشت به منو اصلی
              </li>
